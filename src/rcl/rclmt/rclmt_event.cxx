...
void Event::SignalIn(double millis) {
	...
	const auto result = SetWaitableTimer(d_handle,
	                                     reinterpret_cast<const LARGE_INTEGER*>(&t),
	                                     0,
	                                     nullptr,
	                                     nullptr,
	                                     0);
...
void Event::SignalEvery(double millis) {
	const int64_t now{0};
	const auto period = static_cast<LONG>(millis);
	const auto result = SetWaitableTimer(d_handle,
	                                     reinterpret_cast<const LARGE_INTEGER*>(&now),
	                                     period,
	                                     nullptr,
	                                     nullptr,
	                                     0);
...
Event Event::MakeEvent(bool initialState /*=false*/) {
	const auto event = CreateEventW(nullptr,
	                                /*bManualReset=*/FALSE,
	                                static_cast<BOOL>(initialState),
	                                nullptr);
	if (event == nullptr) {
		const auto error = GetLastError();
...
Event Event::MakeTimer() {
	const auto event = CreateWaitableTimerW(nullptr,
	                                        /*bManualReset=*/FALSE,
	                                        nullptr);
	if (event == nullptr) {
		const auto error = GetLastError();
